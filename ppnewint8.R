...
# with wage differentials and allowing for
# unproductive sectors

ppnewint7 <- function(A, Ap, l, lp, w, wp, v, Q, Qp, D, Dp, K, t, lp_simple){
  ...
  # wp (1xm): vector of wage rates for productive industries
  ...
  if(v>=(lp_simple%*%(diag(wp))%*%Qp)/(l%*%(diag(w))%*%Q)){
    ...
    myfunc <- function(r2){
      ...
      return(
        (C1%*%B1%*%y) - ((lp_simple%*%diag(wp))%*%Qp)/v
      )
    }
